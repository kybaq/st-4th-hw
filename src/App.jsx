...
function App() {
  ...
  // NOTE:
  const textFromStorage = window.localStorage.getItem("texts");
  const initialState = textFromStorage ? Array(textFromStorage) : []; // truthy 값이거나 or null 반환;

  const [texts, setTexts] = useState(initialState);

  useEffect(() => {
    // TODO: 상태가 변경될 때마다 로컬 스토리지에 저장. key 값은 texts 로 합시다.
    window.localStorage.setItem("texts", texts);
  }, [texts]);
